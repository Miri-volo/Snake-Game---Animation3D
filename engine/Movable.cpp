...
void Movable::RotateInZXZ(const Eigen::Vector3f& angles)
{
    if (isStatic) return;
    auto aggregatedRotation = Tout.rotation();
    auto rotatedZXZAngles = aggregatedRotation.eulerAngles(2, 0, 2) + angles;
    auto z1 = Eigen::AngleAxisf(rotatedZXZAngles[0], Eigen::Vector3f::UnitZ());
    auto x = Eigen::AngleAxisf(rotatedZXZAngles[1], Eigen::Vector3f::UnitX());
    auto z2 = Eigen::AngleAxisf(rotatedZXZAngles[2], Eigen::Vector3f::UnitZ());
    Tout.rotate(aggregatedRotation.transpose() * z1 * x * z2);
    PropagateTransform();
}
...
Eigen::Matrix4f Movable::GetTransform() const
{
    return Tout.matrix() * Tin.matrix();
}
